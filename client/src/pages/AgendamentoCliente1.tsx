...
import { CalendarDays, CheckCircle2, Clock, Scissors, User, UserRound } from "lucide-react";
...
export default function AgendamentoCliente() {
  ...
  const totalValor = servicosEscolhidos.reduce((s, item) => s + Number(item.valor || 0), 0);
  const totalDuracao = servicosEscolhidos.reduce((s, item) => s + duracaoParaMinutos(item.duracao), 0);
  ...
  useEffect(() => {
    ...
    api(
      ...
      .then((dados) => setHorarios(dados?.horarios || []))
  ...
  const solicitar = async () => {
    try {
      ...
      if (!servicosSelecionados.length) throw new Error("Escolha pelo menos um serviço.");
      ...
      setEnviando(true);
      await api("/api/publico/agendamento/solicitar", {
        method: "POST",
        body: JSON.stringify({
          nome: cliente.nome,
      ...
      setMensagem("Solicitação enviada com sucesso. Aguarde a confirmação do profissional.");
      setHora("");
      ...
      <div className="mx-auto flex min-h-screen max-w-6xl flex-col px-4 py-8 md:px-8">
        <header className="mb-8 flex flex-col gap-4 rounded-3xl border border-white/10 bg-white/[0.04] p-5 shadow-2xl md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-bold text-amber-400">Agendamento online</p>
              <h1 className="text-2xl font-black md:text-4xl">
                {empresa.nome || empresa.marca || "Agende seu horário"}
              </h1>
              <p className="mt-1 text-sm text-slate-300 md:text-base">
                Solicite seu atendimento. A confirmação será feita pelo profissional.
              </p>
            </div>
          </div>
          <div className="rounded-2xl border border-amber-400/30 bg-amber-400/10 px-4 py-3 text-sm text-amber-100">
            Os horários ocupados aparecem apenas como indisponíveis.
          </div>
        </header>
        ...
        <main className="grid flex-1 gap-6 lg:grid-cols-[1fr_380px]">
          <section className="rounded-3xl border border-white/10 bg-white p-5 text-slate-950 shadow-2xl md:p-6">
            ...
            {carregando ? (
              <div className="rounded-2xl bg-slate-100 p-6 text-center text-slate-500">Carregando...</div>
            ) : (
              <div className="space-y-5">
                        ...
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              ...
                              <p className="mt-1 text-xs text-slate-500">Duração: {s.duracao || "30 min"}</p>
                            </div>
                            <div className="text-right">
                              <p className="font-black text-amber-700">{dinheiro(s.valor)}</p>
                              <p className="mt-1 text-xs font-bold text-slate-500">{selecionado ? "Selecionado" : "Selecionar"}</p>
                            </div>
                          </div>
                        </button>
                      );
                ...
                <div className="rounded-2xl border bg-slate-50 p-4">
                  ...
                  {carregandoHorarios ? (
                    <div className="rounded-xl bg-white p-5 text-center text-slate-500">Carregando horários...</div>
                  ) : horarios.length ? (
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5">
                      {horarios.map((h) => (
                        <button
                          ...
                          onClick={() => h.disponivel && setHora(h.hora)}
                    ...
                    <div className="rounded-xl bg-white p-5 text-center text-slate-500">
                      Escolha serviço(s), profissional e data para ver os horários.
                    </div>
                  )}
                </div>
              </div>
          ...
          <aside className="rounded-3xl border border-white/10 bg-white/[0.04] p-5 shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-slate-200">
                ...
                <p>Serviço(s): <b>{servicosEscolhidos.length ? servicosEscolhidos.map((s) => s.nome).join(" + ") : "Nenhum selecionado"}</b></p>
                <p>Profissional: <b>{profissionalSelecionado?.nome || "Não selecionado"}</b></p>
                <p>Data: <b>{data}</b></p>
                <p>Hora: <b>{hora || "Não selecionado"}</b></p>
                <p>Duração total: <b>{totalDuracao || 0} min</b></p>
                <p>Valor total: <b>{dinheiro(totalValor)}</b></p>
              </div>

              {erro && <div className="rounded-xl border border-red-400/30 bg-red-500/10 p-3 text-sm text-red-200">{erro}</div>}
              {mensagem && <div className="rounded-xl border border-emerald-400/30 bg-emerald-500/10 p-3 text-sm text-emerald-200">{mensagem}</div>}
              ...
              <div className="flex items-start gap-2 text-xs text-slate-400">
                ...
                <span>Após solicitar, o profissional confirma o horário. Você receberá a confirmação pelo WhatsApp.</span>
              </div>
            </div>
          </aside>
        </main>
