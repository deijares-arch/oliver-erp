...
export function registerApiRoutes(app: Express) {
  ...
  app.post('/api/auth/login', async (req, res) => {
    try {
      ...
      const rows = await query(
        'SELECT * FROM usuarios WHERE empresa_id = ? AND email = ? AND senha = ? LIMIT 1',
  ...
  app.get('/api/usuarios-pendentes', async (_req, res) => {
    try {
      await ensureAuthTables();
      const rows = await query(
        "SELECT id, nome, email, telefone, perfil, ativo, status, criado_em FROM usuarios WHERE empresa_id = ? AND status = 'PENDENTE' ORDER BY criado_em DESC, id DESC",
        [EMPRESA_ID]
      );
  ...
  app.get('/api/usuarios', async (_req, res) => {
    try {
      await ensureAuthTables();
      const rows = await query(
        "SELECT id, nome, email, telefone, perfil, ativo, status, criado_em FROM usuarios WHERE empresa_id = ? AND status <> 'PENDENTE' ORDER BY nome",
        [EMPRESA_ID]
      );
  ...
  app.get("/api/solicitacoes-agendamento", async (_req, res) => {
    try {
      await ensureSolicitacoesAgendamentoTable();
      const rows = await query(
        `SELECT sa.*, s.nome AS servico_nome, p.nome AS profissional_nome
         FROM solicitacoes_agendamento sa
         LEFT JOIN servicos s ON s.id = sa.servico_id
  ...
  app.get("/api/bootstrap", async (_req, res) => {
    try {
      ...
      const [clientes, profissionais, servicos, agendamentos, filaAtendimento, horariosTrabalho] = await Promise.all([
        query("SELECT * FROM clientes WHERE empresa_id = ? AND ativo = 1 ORDER BY id DESC", [EMPRESA_ID]),
        ...
        query(`SELECT a.*, c.nome AS cliente_nome, c.telefone AS cliente_telefone, s.nome AS servico_nome, p.nome AS profissional_nome
               FROM agendamentos a
               ...
               ORDER BY a.data_agendamento DESC, a.hora_agendamento DESC`, [EMPRESA_ID]),
        query(`SELECT f.*, c.nome AS cliente_nome, c.telefone AS cliente_telefone, s.nome AS servico_nome, p.nome AS profissional_nome
               ...
               WHERE f.empresa_id = ?
               ORDER BY f.id DESC`, [EMPRESA_ID]),
        query(`SELECT * FROM horarios_trabalho
               WHERE empresa_id = ?
               ORDER BY FIELD(dia_semana, 'Segunda','Terça','Quarta','Quinta','Sexta','Sábado','Domingo')`, [EMPRESA_ID]),
      ]);
  ...
  app.post("/api/agendamentos", async (req, res) => {
    try {
      ...
      const rows = await query(`SELECT a.*, c.nome AS cliente_nome, c.telefone AS cliente_telefone, s.nome AS servico_nome, p.nome AS profissional_nome
  ...
  app.get("/api/fila", async (_req, res) => {
    try {
      const rows = await query(`SELECT f.*, c.nome AS cliente_nome, c.telefone AS cliente_telefone, s.nome AS servico_nome, p.nome AS profissional_nome
        FROM fila_atendimento f
        LEFT JOIN clientes c ON c.id=f.cliente_id
        LEFT JOIN servicos s ON s.id=f.servico_id
        LEFT JOIN profissionais p ON p.id=f.profissional_id
  ...
  app.get("/api/despesas", async (_req, res) => {
    try {
      ...
      await execute("ALTER TABLE despesas ADD COLUMN IF NOT EXISTS motivo_estorno TEXT NULL").catch(() => undefined);
      await execute("ALTER TABLE despesas ADD COLUMN IF NOT EXISTS data_estorno DATE NULL").catch(() => undefined);
      const rows = await query("SELECT * FROM despesas WHERE empresa_id = ? ORDER BY id DESC", [EMPRESA_ID]);
  ...
  app.get("/api/fluxo-caixa", async (_req, res) => {
    try {
      await ensureFinanceiroColumns();
      const rows = await query("SELECT * FROM fluxo_caixa WHERE empresa_id = ? ORDER BY id DESC", [EMPRESA_ID]);
  ...
  app.get("/api/assinaturas", async (_req, res) => {
    try {
      await ensurePlanosColumns();
      const rows = await query(`SELECT a.*, c.nome AS cliente_nome, c.telefone, p.nome AS profissional_nome, s.nome AS servico_nome,
  ...
  app.post('/api/agendamentos/:id/finalizar', async (req, res) => {
    try {
      ...
      const agRows = await query(
        `SELECT a.*, c.nome AS cliente_nome, c.telefone AS cliente_telefone, p.nome AS profissional_nome, p.comissao
         FROM agendamentos a
         LEFT JOIN clientes c ON c.id = a.cliente_id
         LEFT JOIN profissionais p ON p.id = a.profissional_id
